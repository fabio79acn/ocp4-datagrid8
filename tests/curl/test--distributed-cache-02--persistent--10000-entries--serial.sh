...
readonly myCACHE_DEF_FILE='distributed-cache-02'
[ ! -f $myCACHE_DEF_DIR/${myCACHE_DEF_FILE}.json ] && exit 1
readonly INFINISPAN_CACHE="${myCACHE_DEF_FILE}--persistent--10000-entries--serial"
...
echo -n "Delete cache $INFINISPAN_CACHE if exists. "
...
[[ "$myCURRENT_CACHES" == *"$INFINISPAN_CACHE"* ]] && oc -n ${myPROJ} exec ${myTEST_POD} -- curl       -su ${INFINISPAN_USER}:${INFINISPAN_PASSWORD} --digest -X DELETE           -k "${myDATAGRID}/${INFINISPAN_CACHE}" && echo " Cache $INFINISPAN_CACHE Deleted."
